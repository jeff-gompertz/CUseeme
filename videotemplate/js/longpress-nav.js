...
<head>
  ...
  <title>CUSeeMe — Injected RGB Scan Raster (with Debug, no input)</title>
  <style>
   ...
   /* START: advance button visibility */
#advanceBtn,
.advance-hud-btn {
  position: relative;
  z-index: 1200;               /* higher than HUD windows */
  pointer-events: auto;
...
/* END: advance button visibility / touch fixes */
</style>
</head>
...
<body>
  ...
  <label style="display:flex;align-items:center;gap:8px;">
    <b>Source:</b>
    <select id="sourceSelect" aria-label="Choose source"
            style="min-width:160px;padding:6px;border-radius:8px;background:#ffffffcc;border:1px solid rgba(0,0,0,0.06);">
  ...
  <!-- Advance button: put it here so it sits next to the Source select and camera control -->
  <button id="advanceBtn" class="hud-btn"
          data-longpress-url="https://jeff-gompertz.github.io/CUseeme/"
          aria-label="Hold to advance">
    Hold to advance
  </button>
</div>
...
<script src="/CUseeme/js/longpress-hud.js"></script>

  <script>
(function(){
  // small diagnostic: write results to debugTicker and console
  function log(msg){
    try{
    ...
    try{ console.log('LONGPRESS-DBG:', msg); }catch(e){}
  }

  log('DBG CHECK — starting');

  log('longpress-nav present? ' + (typeof window.makeLongPressNav !== 'undefined'));
  log('longpress-hud script tag present? ' + (!!document.querySelector('script[src*="longpress-hud"]')));
  log('hud-longpress styles injected? ' + (!!document.getElementById('hud-longpress-styles')));
  const btn = document.getElementById('advanceBtn');
  log('advanceBtn found? ' + !!btn);
  if(btn){
    log('advanceBtn classes: ' + (btn.className || '<none>'));
    log('advanceBtn data-longpress-url: ' + (btn.getAttribute('data-longpress-url') || '<none>'));
    log('hud-holdProgress present? ' + !!btn.querySelector('.hud-holdProgress'));
  }
  // Also print if the longpress-hud IIFE threw early (presence of global var unlikely),
